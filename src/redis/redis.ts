...
import { SafeKey, SettlementStore } from '.'
import { AccountServices, setupSettlementServices } from './account-services'
import { ConnectorServices } from '../connector-server'
 ...
 */
export type PrepareSettlement = (
  leaseDuration: number
) => Promise<[BigNumber, DecoratedPipeline]>
...
export interface RedisSettlementEngine {
   ...
   * - Execute the commitment transaction before unconditionally preforming the settlement
...
export const connectRedisStore = async (
  ...
  { sendMessage, notifySettlement }: ConnectorServices,
  ...
  // Setup account services: callbacks such as send message, credit settlement, etc. to pass to the settlement engine
  ...
  const stopCreditLoop = startCreditLoop(redis, notifySettlement)
  ...
  const self: SettlementStore = {
    ...
    async handleSettlementRequest(accountId, idempotencyKey, amount) {
      ...
      const amountQueued = await redis.queueSettlement(
        accountId,
        idempotencyKey,
        amount.toFixed()
      )
